import type { Coords, HadithTokenCluster, MeterOffset, TokenState } from '@/content/levels/types';
...
export const offsetCoords = (origin: Coords, offsetMeters: MeterOffset): Coords => {
    const lat = origin.lat + offsetMeters.z / METERS_PER_DEGREE_LAT;
    const averageLatitude = (origin.lat + lat) / 2;
    const lngScale = Math.max(metersPerDegreeLongitude(averageLatitude), 1);
...
export const getRouteEndpointDistance = (origin: Coords, destination: Coords) =>
    worldDistanceInMeters(origin, destination);

export const getOriginRebaseDelta = (currentOrigin: Coords, focusCoords: Coords) =>
    metersOffsetFromCoords(focusCoords, currentOrigin);
