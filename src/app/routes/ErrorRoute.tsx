...
export const RouteError = () => {
    ...
    const message = isRouteErrorResponse(error)
        ? typeof error.data === 'string'
            ? error.data
            : error.statusText || 'Unknown route failure.'
    ...
    return (
        <main className="flex min-h-screen items-center justify-center p-8">
            <div className="max-w-xl rounded-3xl border border-white/10 bg-white/5 p-8 backdrop-blur">
                ...
                <h1 className="mt-4 font-display text-4xl text-sand-50">
                    {isRouteErrorResponse(error) ? `${error.status} ${error.statusText}` : 'Route Error'}
                </h1>
